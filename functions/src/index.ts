...
export const daily = functions.https.onRequest(async (req, res) => {
  ...
  try {
    if (
      ...
      ((req.query.playlist as string).length !== 22 &&
        (req.query.playlist as string).length !== 0)
    ) {
      res.status(400).json({ message: "Invalid Spotify playlist." });
    } else {
      const dateLocale =
        (req.query.locale as string) || new Date().toDateString();
      const playlistId =
        (req.query.playlist as string) || getDefaultPlaylistId(dateLocale);
    ...
    console.log(e);
    res.status(500).json({ message: "Error getting daily: " + e });
  }
});

// used to reset the cache
// should run on the 15th of every day
export const resetCache = functions.pubsub
  .schedule("0 0 15 * *")
  .onRun((_context) => {
    clearCache();
    return null;
...
const getTracks = async (playlistId: string) => {
  ...
  const playlist =
    cachedPlaylist || (await getSpotifyPlaylist(playlistId, spotifyAuth.token));
  const tracks = cachedPlaylistTracks
    ? cachedPlaylistTracks.tracks
    : await getAllSpotifyPlaylistTracks(playlist, spotifyAuth.token);
